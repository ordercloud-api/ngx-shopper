...
import { MeService, BuyerProduct, LineItem } from '@ordercloud/angular-sdk'
import { map as _map, forEach as _forEach, find as _find, differenceBy as _differenceBy,} from 'lodash';

@Injectable()
export class OcReorderService{
    constructor(
    ...
    order(orderID: string): Observable <orderReorderResponse>{
        return this.ocLineItemService.listAll(orderID)
         .pipe(
             flatMap( list => {
    ...
    getValidProducts(productIds: string[], validProducts?: BuyerProduct[]): Observable <BuyerProduct[]>{
        validProducts =  validProducts || [];
        let chunk = productIds.splice( 0, 25);
        return this.meService.ListProducts( {filters: { ID: chunk.join('|')}})
            .pipe( 
                flatMap(productList =>{
    ...
    isProductInLiValid(products: BuyerProduct[], lineItems: LineItem[] ): Observable<orderReorderResponse>{
    ...
    hasInventory(response: orderReorderResponse): Observable<orderReorderResponse>{
