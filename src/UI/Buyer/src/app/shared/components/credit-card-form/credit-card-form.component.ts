...
export class CreditCardFormComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.yearOptions = Array(20).fill(0).map((_x, i) => `${i + start}`);
    this.monthOptions = Array(12).fill(0).map((_x, i) => (`0${i + 1}`).slice(-2));
  ...
  protected hasRequiredError = (controlName: string) => this.formErrorService.hasRequiredError(controlName, this.cardForm);
}
